...
{
    internal static class ItemTransferApiResponseHelper
    {
        ...
        internal static T? DeserializeOrThrow<T>(HttpResponseMessage response, string? body, string endpoint)
        {
            ...
            catch (JsonException ex)
            {
                throw new InvalidOperationException(
                    $"Item Transfer API response parsing failed for endpoint '{endpoint}'. Response: {BuildSnippet(responseBody)}",
                    ex);
            }
        }
        ...
        {
            throw new InvalidOperationException(
                $"Item Transfer API request failed: {(int)response.StatusCode} ({response.ReasonPhrase})\n" +
                $"Endpoint: {endpoint}\n" +
